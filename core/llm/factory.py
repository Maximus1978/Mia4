...
def sweep_idle(idle_config: dict[str, int] | None = None, now: float | None = None) -> List[Tuple[str, str]]:  # noqa: D401
    ...
    for mid, prov in list(llm_mod._providers.items()):  # type: ignore[attr-defined]
...
def get_model(model_id: str, repo_root: str | Path = ".", skip_checksum: bool = False) -> ModelProvider:  # noqa: D401
    llm_mod = get_module_manager().get("llm")
    return llm_mod.get_provider(model_id, repo_root=repo_root, skip_checksum=skip_checksum)
...
def get_model_by_role(role: str, repo_root: str | Path = ".", skip_checksum: bool = False) -> ModelProvider:  # noqa: D401
    llm_mod = get_module_manager().get("llm")
    return llm_mod.get_provider_by_role(role, repo_root=repo_root, skip_checksum=skip_checksum)


def apply_reasoning_overrides(base_kwargs: dict[str, object], mode: str | None) -> dict[str, object]:  # noqa: D401
    llm_mod = get_module_manager().get("llm")
